module.exports = class RESTclient {
  ...
  getUser($) {
    if (/^\d+$/.test($) && this.bot.users.get($)) return this.bot.users.get($);
    else if (/^<@!?(\d+)>$/.test($)) {
      const u = this.bot.users.get($.match(/^<@!?(\d+)>$/)[1]);
      if (u) return u;
    }else if (/^(.+)#(\d{4})$/.test($)) {
      const m = $.match(/^(.+)#(\d{4})$/);
      const u = this.bot.users.filter((u) => u.username === m[1] && Number(u.discriminator) === Number(m[2]));
      if (u.length > 0) return u[0];
    } else {
      const u = this.bot.users.filter((u) => u.username.toLowerCase().includes($.toLowerCase()));
      if (u.length > 0) return u[0];
    }
  ...
  getChannel($, g) {
    ...
    if (/^\d+$/.test($)) {
      if (g) {
        if (!g.channels.has($)) return undefined;
      ...
      const m = $.match(/^<#(\d+)>$/);
      if ($) {
        if (!g.channels.has(m[1])) return undefined;
